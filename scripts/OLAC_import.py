...
"""
OLAC_import.py
Full OLAC Ingestion Pipeline for Myaamia/Algonquian Languages
Supports harvest, extraction, BabelEdit JSON, LEMON RDF, and basic Hunspell.
"""
...
class OLACImporter:
    def __init__(self, db_path: str = 'data/olac_data.db'):
        ...
        self.xslt_dir = Path("XSLT/linguistics-suite/xslt")
    ...
    def harvest_olac(self, max_records: int = 300):
        ...
        print(f"✅ Harvested {len(all_items)} records.")
    ...
    def export_babeledit_json(self, output_dir: str = "babeledit_algonquian"):
        ...
        conn.close()
        print(f"✅ BabelEdit files ready in: {out_dir}")
    ...
    def export_lemon_rdf(self, output_ttl: str = "output/myaamia_lexicon.ttl"):
        ...
        for lang_code, word, concept in cursor.fetchall():
            if not lang_code or not word: continue
            ...
            if lang_code in ["fr_old", "la", "en"]:
                g.add((sense_uri, RDFS.comment, Literal(f"Supervisor: {self.LANGUAGE_MAP.get(lang_code)}")))
        ...
        print(f"✅ LEMON RDF exported:")
        print(f"   Turtle : {out_path}")
    ...
    def basic_hunspell_prep(self, output_dic: str = "hunspell/myaamia.dic"):
        ...
        with open(output_dic, "w", encoding="utf-8") as f:
            ...
            for w in sorted(set(words)):
                f.write(f"{w}\n")
        print(f"✅ Hunspell .dic created: {output_dic} ({len(words)} entries)")

# CLI entry point for the skill
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python OLAC_import.py <command> [--max N] [--output DIR]")
        sys.exit(1)
    ...
    if cmd == "harvest":
        max_r = int(sys.argv[sys.argv.index("--max")+1]) if "--max" in sys.argv else 300
        importer.harvest_olac(max_records=max_r)
    elif cmd == "extract":
        importer.extract_lexical()
    elif cmd == "babeledit":
        out = sys.argv[sys.argv.index("--output")+1] if "--output" in sys.argv else "babeledit_algonquian"
        importer.export_babeledit_json(out)
    elif cmd == "lemon":
        out = sys.argv[sys.argv.index("--output")+1] if "--output" in sys.argv else "output/myaamia_lexicon.ttl"
        importer.export_lemon_rdf(out)
    elif cmd == "hunspell":
